...
void loadJson(Circom_CalcWit *ctx, const char *json_buffer, unsigned long buffer_size) {
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    if (v.size() != signalSize) {
      ...
      errStrStream << "Error loading signal " << it.key() << ": Expected " << signalSize << " values, Receoved " << v.size() << "\n";
...
void storeBinWitness(Circom_CalcWit *ctx, char *buffer) {
    ...
    // std::cout << "Finsihed creating withness file headers, on to data: " << Nwtns <<" \n";
     for (int i=0;i<Nwtns;i++) {
         ctx->getWitness(i, &v);
         Fr_toLongNormal(&v, &v);
        // std::cout << "value : "<< i << v.longVal << " \n";
